...
def load_darcy_flow_small(n_train, n_tests,
    ...
    path = Path("").resolve().parent.joinpath('/content/drive/MyDrive/Andy')
    return load_darcy_pt(str(path),
                         n_train=n_train, n_tests=n_tests,
                         batch_size=batch_size, test_batch_sizes=test_batch_sizes,
                         test_resolutions=test_resolutions, train_resolution=1024,
...
def load_darcy_pt(data_path,
    ...
    data = torch.load(Path(data_path).joinpath(f'200K_training.pt').as_posix())
    ...
    data = torch.load(Path(data_path).joinpath(f'200K_testing.pt').as_posix())
