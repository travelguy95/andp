...
def collide(p1,p2):
    ...
    if length < target and length > 0:
      ...
      p1.x = p1.x - p1.radius
      p2.x = p2.x + p2.radius
@jit((Particle.class_type.instance_type,int64,int64,float64,float64))
def border_collide(self,rows,cols,dt,top):
...
def run_code(rows,cols,steps,obstruct=np.array([[]]),obstruct_u=np.array([[]]),obstruct_v=np.array([[]])):
  ...
  for iter in range(0,steps+1):
    ...
    for times in range(0,60): # do this 10 times
      for i, particle in enumerate(my_particles):
        row = math.floor(particle.y)
        col = math.floor(particle.x)
        if col > cols-1:
        ...
        for particle2 in neighbor_particles2:
          ...
          border_collide(particle,rows,cols,dt,top)
          border_collide(particle2,rows,cols,dt,top)
    for count in range(0,1):
      for i, particle in enumerate(my_particles):
        border_collide(particle,rows,cols,dt,top)
    list_of_list_positions = []
    ...
    for i, particle in enumerate(my_particles):
      ...
      if x > cols + 1:
        particle.x = cols + 1 - particle.radius
        x = cols + 1 - particle.radius
      if y > rows + 1:
        particle.y = rows + 1 - particle.radius
        y = rows + 1 - particle.radius
      dx = x-math.floor(x)
